...
ContentPlugin.prototype.create = function (data, next) {
  ...
  database.getDatabase(function (error, db) {
    ...
    if (!data._id && !data._courseId) {
      ...
      const resource = permissions.buildResourceString(user.tenant._id, `/api/global/content/${permissionQuery._type}`);
      permissions.hasPermission(user._id, 'create', resource, function (error, isAllowed) {
        if (!isAllowed) {
          return next(new ContentPermissionError());
        }
...
ContentPlugin.prototype.retrieve = function (search, options, next) {
  ...
  database.getDatabase(function (error, db) {
    ...
    // Ensure the tags are populated, but only for models that actually define
    // a tags path (newer mongoose enforces strictPopulate by default and
    // throws when populating a path that isn't in the schema)
    var Model = db.getModel && db.getModel(self.getModelName());
    if (Model && Model.schema && Model.schema.path('tags')) {
    ...
    db.retrieve(self.getModelName(), search, options, function (err, records) {
      ...
      async.each(records, function (contentItem, callback) {
        ...
        const resource = permissions.buildResourceString(user.tenant._id, `/api/global/content/${permissionQuery._type}`);
        permissions.hasPermission(user._id, 'read', resource, function (error, isAllowed) {
...
ContentPlugin.prototype.update = function (search, delta, next) {
  ...
  const resource = permissions.buildResourceString(user.tenant._id, `/api/global/content/${permissionQuery._type}`);
  permissions.hasPermission(user._id, 'update', resource, function (error, isAllowed) {
    if (!isAllowed) {
      return next(new ContentPermissionError());
    }
...
ContentPlugin.prototype.destroy = function (search, force, next) {
  ...
  self.retrieve(search, {}, function (err, results) {
    ...
    permissionQuery._type = permissionQuery._type || self.getModelName();
    const resource = permissions.buildResourceString(user.tenant._id, `/api/global/content/${permissionQuery._type}`);
    permissions.hasPermission(user._id, 'delete', resource, function (error, isAllowed) {
      if (!isAllowed) {
        return next(new ContentPermissionError());
      }
