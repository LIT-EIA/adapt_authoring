...
// newer mongoose enforces strictPopulate by default, throwing when populating a
// path that isn't defined in the target schema. The app relies on populating
// paths (e.g. 'tags') that don't exist on every content model, so restore the
// previous, lenient behaviour globally.
...
MongooseDB.prototype.connect = function(db) {
  ...
  authenticationString = dbUser && dbPass ? encodeURIComponent(dbUser) + ':' + encodeURIComponent(dbPass) + '@' : '';
  ...
  var conn = mongoose.createConnection(connectionString, options);

  return conn.asPromise().then(function(conn) {
    this.conn = conn;
    ...
    this._models = {};
    this.conn.db.command({ buildInfo: 1 }).then((info) => {
...
MongooseDB.prototype.retrieve = function(objectType, search, options, callback) {
  ...
  // newer mongoose versions require the filter to be a plain object (null/undefined are rejected)
  ...
  if (operators && 'object' === typeof operators) {
    if (operators.sort && 'object' === typeof operators.sort) {
      // newer mongoose strictly validates sort values (only accepts the number
      // 1/-1 or 'asc'/'ascending'/'desc'/'descending'); normalise numeric
      // strings (e.g. "1"/"-1" from query params) which older mongoose tolerated
      sort = {};
      Object.keys(operators.sort).forEach(function (key) {
        var val = operators.sort[key];
        if (val === '1' || val === '-1') {
          val = Number(val);
        }
        sort[key] = val;
  ...
  if (Model = this.getModel(objectType)) {
    ...
    if (distinct) {
      ...
      query = Model.find({}).elemMatch(elemMatch.collection, elemMatch.query);
    } else {
    ...
    var execQuery = jsonOnly ? query.lean() : query;

    execQuery.exec().then(function (results) {
...
MongooseDB.prototype.update = function(objectType, conditions, updateData, callback) {
  var Model = false;
  if (!conditions || 'object' !== typeof conditions) {
    conditions = {};
  }
  if (Model = this.getModel(objectType)) {
    Model.findOne(conditions).exec().then(function (doc) {
      if (!doc) {
...
MongooseDB.prototype.destroy = function(objectType, conditions, callback) {
  ...
  if (Model = this.getModel(objectType)) {
    Model.deleteMany(conditions).exec().then(function (result) {
      return callback(null, result);
    }, function (err) {
      return callback(err);
    });
