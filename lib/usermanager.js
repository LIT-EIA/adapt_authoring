...
exports = module.exports = {
  ...
  retrieveUser: function (search, options, callback) {
    ...
    // guard against ambiguous queries: an undefined value in the search filter
    // is silently dropped during BSON serialization, which can unintentionally
    // widen the match to more than one document (e.g. an omitted req.body.email)
    for (var key in search) {
      if (Object.prototype.hasOwnProperty.call(search, key) && search[key] === undefined) {
        return callback(new Error('retrieveUser: search value for "' + key + '" must be defined'));
      }
    }
  ...
  clearOtherSessions: function (req, userId) {
    var self = this;
    self.clearPassportSessions(req, userId);
    self.clearMfaTokens(req, userId);
  },

  clearPassportSessions: function (req, userId) {
    ...
    database.getDatabase(function (err, db) {
      ...
      db.retrieve('session', { "session.passport.user._id": new ObjectId(user_id), "_id": { $ne: req.sessionID } }, { jsonOnly: true }, function (err, results) {
        if (err) {
          logger.log('error', err);
        }
        if (!results && !results.length) {
  ...
  clearMfaTokens: function (req, userId) {
    var auth = require('./auth');
    database.getDatabase(function (err, db) {
      var cookieKey = app.configuration.getConfig('devEnv') ? `connect-${app.configuration.getConfig('devEnv')}.fid` : 'connect.fid'
      var user_id = req && req.session.passport && req.session.passport.user ? req.session.passport.user._id : userId;
        ...
        if (tokenId) {
          auth.validateTokenIdSignature(tokenId, function(error, token){
            if(err){
              ...
              query.tokenId = { $ne: tokenId };
            }
          })
        } else {
          query.sessionId = { $ne: req.sessionID };
        }
      }
      db.retrieve('mfatoken', query, { jsonOnly: true }, function (error, results) {
        if (error) {
          logger.log('error', error);
          return next(error);
        }
        if (!results && !results.length) {
          logger.log('error', 'An error occured while clearing other existing mfa tokens');
        } else {
          results.forEach(function (document) {
            db.destroy('mfatoken', { _id: document._id }, function (error) {
              if (error) {
                logger.log('error', 'An error occured while deleting a mfa token')
              }
            })
          })
        }
  ...
  createUserPasswordReset: function (userReset, next) {
    ...
    this.retrieveUser({ email: new RegExp(userReset.email, 'i'), auth: 'local' }, function (error, user) {
      if (error) {
        return next(error);
      } else if (user) {
        if (user.passwordResetCount >= configuration.getConfig('maxLoginAttempts')) {
  ...
  /**
    ...
    * @param {function} callback - function of the form function (error, user)
    */
  createMfaToken: function (user, req, token, tokenId, next) {
    var self = this;
    // schema defines email as required, but for searching check that email is defined
    if (!user.email || 'string' !== typeof user.email) {
      return next(new UserEmailError('User email is required!'));
    ...
    this.retrieveUser({ email: user.email, auth: 'local' }, function (error, user) {
      if (error) {
        return next(error);
      } else if (user) {
        database.getDatabase(function (err, db) {
          ...
          db.retrieve('mfatoken', { userId: user._id }, function (error, results) {
            if (error) {
              logger.log('error', error);
              return next(error);
            } else {
              self.deleteExpiredMfaToken(results, function () {
                // Create mfa token
                user.user = user._id;
                var delta = {
                  userId: user._id,
                ...
                db.create('mfatoken', delta, function (error, result) {
                  if (error) {
                    logger.log('error', 'Failed to create new token: ', delta);
                    return next(error);
                  } else {
                    user.mfaResetCount++;
                    db.update('user', { _id: user._id }, { mfaResetCount: user.mfaResetCount }, function (error, result) {
                      if (error) {
                        logger.log('error', 'Failed to update user mfa reset count for user: ', user);
                        return next(error);
                      }
                    });
                    var currentDate = new Date();
                    Moment.locale('en');
                    var dateEN = Moment(currentDate).format('MMMM Do YYYY');
                    Moment.locale('fr');
                    var dateFR = Moment(currentDate).format('Do MMMM YYYY');
                    emailTemplate = {
                      email: user.email,
                      template: 'loginMfa',
                      personalisation: {
                        name: user.firstName,
                        verificationCode: result.validationToken,
                        dateFR: dateFR,
                        dateEN: dateEN
                      }
                    }
  ...
  validateMfaToken: function (token) {
    var currentDate = new Date();
    var THIRTY_DAYS = 30 * 24 * 60 * 60 * 1000;
    var tokenExpired = token.validationDate === null ? false : (currentDate - token.validationDate) > THIRTY_DAYS;
    return (token.verified === true && !tokenExpired);
  },

  /**
   * Retrieves a single mfa token
   *
   * @param {object} search - fields to match: should use 'token' which is unique
   * @param {function} callback - function of the form function (error, userReset)
   */
  retrieveMfaToken: function (search, callback) {
    database.getDatabase(function (err, db) {
      if (err) {
        logger.log('error', error);
        return callback(error)
      ...
      db.retrieve('mfatoken', search, function (error, results) {
        if (error) {
          logger.log('error', error);
          return callback(error);
        } else {
  ...
  updateMfaToken: function (search, update, callback) {
    database.getDatabase(function (err, db) {
      if (err) {
        logger.log('error', error);
        return callback(error)
      }
      db.update('mfatoken', search, update, function (error, results) {
        if (error) {
          logger.log('error', error);
          return callback(error);
        } else {
  ...
  /**
   * Deletes a single mfa token
   *
   * @param {object} user - must match the user in db
   * @param {function} callback - function of the form function (error)
   */
  deleteMfaToken: function (data, callback) {
    // confirm the user password reset exists and is there is only one of them
    return database.getDatabase(function (err, db) {
      if (err) {
        return callback(err);
      }
      db.destroy('mfatoken', data, callback);
    }, configuration.getConfig('dbName'));
  },

  deleteExpiredMfaToken: function (results, callback) {
    var self = this;
    if (results && results.length) {
      results.forEach(function (result) {
        if (!self.validateMfaToken(result)) {
          self.deleteMfaToken(result, function (error) {
            if (error) {
              logger.log('error', 'An error occured while deleting an mfa token')
            }
          })
        }
      });
  ...
  logAccess: function (user, callback) {
    ...
    delta.lastLogin = currentTime;

    // Reset the count of any failed logins / reset password / failed mfa / reset mfa
  ...
  getUserDetails: function (user, callback) {
    ...
    database.getDatabase(function (err, db) {
      db.retrieve('user', query, function (error, result) {
        if (error) {
          return callback(error);
        }
  ...
  init: function (app) {
      ...
      if (usr) {
        self.retrieveUser({ _id: usr._id }, function (err, usr) {
          ...
          self.getUserRoles(usr, function (err, roles) {
            ...
            // refactor in the future - workaround to strip protected properties for security purposes
            const userJson = JSON.stringify(usr);
      ...
      if (delta.password) {
        self.validatePassword(delta.password, delta, function (result) {
          if (result.length > 0 && delta.password.length < 12) {
            return res.status(500).json('app.passwordindicatormedium');
          }
          else if (result.length > 0) {
            return res.status(500).json('app.passwordreused');
          }
          ...
          self.hasPermission('update', user, '/api/user/resetpassword', function (err, isAllowed) {
            ...
            // sanitise the input
            delta = { password: delta.password, email: email };
    ...
    rest.post('/user', function (req, res, next) {
      ...
      userData.password = crypto.randomBytes(64).toString('hex');
