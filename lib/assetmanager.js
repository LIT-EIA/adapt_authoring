...
exports = module.exports = {
  ...
  putAsset: function (req, res, next) {
    ...
    self.updateAsset({ _id: req.body._id }, asset, function (error, assetRec) {
      ...
      if (error) {
        res.statusCode = 500;
        return res.json({ success: false, message: error && error.message || 'An error occurred while saving the asset.' });
      }
  ...
  deleteAsset: function (req, res, next) {
    ...
    this.destroyAsset(req.params.id, function (error) {
      if (error) {
        res.statusCode = 500;
        return res.json({ success: false, message: error && error.message || 'An error occurred while deleting the asset.' });
      }
