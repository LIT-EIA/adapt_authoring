...
var path = require('path'),
    ...
    formidableHelpers = require('./formidableHelpers'),
    bytes = require('bytes');
...
function AssetNotFoundError (message, assetID) {
  ...
  this.message = (message || 'Asset was not found');
  if (assetID) {
    this.message += ' (Asset ID: ' + assetID + ')';
  }
};
...
exports = module.exports = {
  ...
  trashAsset: function(id, next) {
    ...
    self.hasPermission('delete', user._id, tenantId, { _id: id}, function (err, isAllowed) {
      if (!isAllowed) {
        const error = new AssetPermissionError();
        error.status = 403;
        return next(error);
      }
  ...
  postAsset: function (req, res, next) {
    ...
    form.parse(req, function (error, fields, files) {
      ...
      // formidable v3 renamed File#type to File#mimetype
      var openedFileType = form.openedFiles[0] && (form.openedFiles[0].mimetype || form.openedFiles[0].type);
      var maxUpload = bytes(findValueByKey(customTypesData, openedFileType));
      if (maxUpload === null){
