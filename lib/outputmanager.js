...
OutputPlugin.prototype.removeBuildIncludes = async (configPath, next) => {
  // NOTE: `next` is intentionally called once, after this try/catch has fully
  // completed (not from within it). Calling it from inside the try block risks
  // a synchronous chain of downstream callbacks (e.g. via async.waterfall)
  // throwing back into this function's own catch block, which would then call
  // `next` a second time ("Callback was already called").
  let error = null;
  try {
...
OutputManager.prototype.setupRoutes = function () {
  ...
  // STORYBOARD OUTPUT
  rest.get('/output/:type/storyboard/:courseid', function(req, res, next) {
    var type = req.params.type;
    var courseId = req.params.courseid;
    var mode = Constants.Modes.Storyboard;

    that.storyboard(type, courseId, req, res, function(error, result) {
      if (error) {
        logger.log('error', error);
        return res.json({ success: false, message: error.message });
      }
...
// STORYBOARD HANDLER
OutputManager.prototype.storyboard = function () {
  var args = Array.prototype.slice.call(arguments);
  var type = args.shift();
  var cb = args[args.length - 1];
  ...
  this.getOutputPlugin(type, function (error, plugin) {
    ...
    // force insert "mode" before req
    // args = [courseId, req, res, callback]
    // we need [courseId, Constants.Modes.Storyboard, req, res, callback]
    ...
    return plugin.storyboard(courseId, Constants.Modes.Storyboard, req, res, callback);
  });
};
