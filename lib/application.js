...
Origin.prototype.createServer = function (options, cb) {
  var server = express();
  // Express 5 defaults to the 'simple' query parser (Node's querystring.parse),
  // which returns null-prototype objects and breaks existing code that calls
  // req.query.hasOwnProperty(...) etc. Restore Express 4's 'extended' (qs-based) behaviour.
  ...
  */
  if (options.minimal) {
    ...
    // newer body-parser leaves req.body as undefined for bodyless requests (e.g. GET);
    // restore the previous guarantee that req.body is always an object.
  ...
  */
  ...
  require('./database').getDatabase(function (err, db) {
    ...
    //Adds middleware to log last user access
    server.use(function (req, res, next) {
      var timeToStore = 60000;
      if (req.user && req.user.id) {
        var userId = req.user.id;
        var now = Date.now();
        var sessionLast = req.session.lastActivityUpdate || 0;
        var tempLast = app.tempActivityMap.get(userId);
        var shouldLog = !tempLast && (now - sessionLast > timeToStore);

        if (shouldLog) {
          // Set memory cache for block burst requests
          app.tempActivityMap.set(userId, now);
          // Log user activity and update session
          req.session.lastActivityUpdate = now;
          req.session.save(function () {
            usermanager.logActivity(req.user);
          });
          // Clean up memory cache after x seconds
          setTimeout(function () {
            app.tempActivityMap.delete(userId);
          }, timeToStore);
        }
      }
      next();
...
Origin.prototype.run = function (options) {
  ...
  this.once('modulesReady', function (app) {
    try {
      ...
      var phraseStringsFr = JSON.parse(fs.readFileSync(path.join(configuration.serverRoot, 'routes', 'lang', 'fr.json'), { encoding: 'utf8' }));
