...
exports = module.exports = {
  ...
  createMfaToken: function (cb) {
    require('crypto').randomInt(0, 10000, function (ex, code) {
      return cb(null, String(code).padStart(4, '0'));
    })
  },
      ...
      if (modloaded === WAITFOR) {
        ...
        rest.post('/newLoginMfaToken', function (req, res, next) {
          usermanager.retrieveUser({ email: req.body.email, auth: 'local' }, function (error, user) {
            ...
            if (user) {
              if (user.mfaResetCount >= 3) {
                logger.log('error', 'mfa resent more than 3 times, email locked!');
                res.statusCode = 401;
        ...
        rest.get('/authcheck', function (req, res, next) {
          ...
          usermanager.retrieveMfaToken({
            ...
            if (data && data.length) {
              ...
              var result = {
                isAuthenticated: (user && user._id && tokenVerified && !tokenExpired),
