    ...
    fs.readdir(path.join(configuration.serverRoot, 'lib', 'role'), function (err, files) {
      if(options && options.roles){
        options.roles.forEach(function(role){
          var roledef = false;
          try {
            roledef = require(path.join(configuration.serverRoot, 'lib', 'role', role));
          } catch (error) {
            console.log(error);
          }
    ...
    fs.readdir(path.join(configuration.serverRoot, 'lib', 'role'), function (err, files) {
      async.each(
        ...
        function (file, callback) {
          ...
          logger.log('info', 'Updating ' + role.name + ' role')
          db.update('role', { name: role.name }, { version: role.version, statement: role.statement, default: role.default }, function (err, rec) {
            if (err) {
              logger.log('warn', '- ' + role.name + ' update failed', err);
            }
            if (!rec) {
              logger.log('warn', '- ' + role.name + ' role doesn\'t exist, cannot update so creating record', err);
              installDefaultRoles({roles: [file]});
            }
            return callback(err);
          });
        },
