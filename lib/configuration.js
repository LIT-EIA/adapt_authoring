...
Configuration.prototype.load = function (filePath, callback) {
  ...
  if (process.env.serverPort) {
    var configs = parseEnv(process.env);
    config.conf = _.extend(config.conf, configs);
...
Configuration.prototype.getAllClientSideConfigs = function () {
  ...
  var mimeTypeKeys = Object.keys(configs).filter(function (key) {
    return regex.test(key);
  });

  var COMBINED_ALLOWED_CLIENT_SIDE_KEYS = ALLOWED_CLIENT_SIDE_KEYS.concat(mimeTypeKeys);

  // Only return allowed values
  return COMBINED_ALLOWED_CLIENT_SIDE_KEYS.reduce((result, key) => {
